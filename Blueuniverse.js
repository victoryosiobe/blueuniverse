...
mainContent.addEventListener('click', updatePara)
...
function updatePara() {
  for (eachpara of mainContent) eachpara.style.border === '0.3em solid blue' ? eachpara.style.border = '0.1em solid blue' : eachpara.style.border = '0.3em solid blue'
}
...
function updatebutt() { // to delay 
  let butt = document.getElementById('butt')
  butt.style.height = '3em'
  butt.style.lineHeight = '3em'
  body.style.background = 'linear-gradient(to bottom right, #0F1C60, #040924)'
}

function updateAlwaysFormat() {
  let astylo = always.style
  astylo.color = 'blue'
  astylo.background = 'white'
  astylo.textAlign = 'left'
  astylo.fontWeight = 'bold'
  astylo.borderBottom = '.1em solid blue'
}
...
function reveal() {
  ...
  let percent90 = bodyHeight * 0.9
  windowViewAt > percent10 ? arrowMan.style.display = 'block' : arrowMan.style.display = 'none'
  windowViewAt < percent90 ? transformArrow('downward') : transformArrow('upward')
  for (eachpara of mainContent) {
    ...
    let revealpoint = 70
    revealtop < windowheight - revealpoint ? eachpara.style.transform = 'translateY(0%)' : eachpara.style.transform = 'translateY(50%)' //lets say windowheight is 700 
...
more.addEventListener('click', function(e) { //function will still work, both for 'e' and the code under. see why e was added in the next code line.
  ...
  if (moreman.style.display == 'none') {
    ...
    let timed = setInterval(function() { //tried to do a fadeout after clicking again.
      if (moreman.style.opacity <= 0) { //if it's less or equal to 0 after reduction in opacity, clear interval and set display as none
        clearInterval(timed)
...
''
document.addEventListener('click', function(event) { //this is literally for making the menu hide when anywhere when the document is clicked, but give it some fadeout impression too.
  // console.log('user clicked: ', event.target)
  if (!moreman.contains(event.target)) { //meaning, if moreman does not contains the event target exceute code
